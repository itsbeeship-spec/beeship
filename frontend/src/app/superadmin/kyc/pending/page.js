...
export default function PendingVerificationPage() {
  ...
  const fetchKYCList = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const q = new URLSearchParams({ page: String(page), limit: "100", status: "PENDING" });
      ...
      console.error("Failed to load KYC pending list:", err);
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderDocBadges = (u) => {
    return (
      <div className="flex flex-wrap gap-1.5">
        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold ${u.aadhaarNumber ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-800 text-slate-500"}`}>
          Aadhaar {u.aadhaarNumber ? "✓" : "—"}
        </span>
        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold ${u.panNumber ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-800 text-slate-500"}`}>
          PAN {u.panNumber ? "✓" : "—"}
        </span>
        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold ${u.gstNumber ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-800 text-slate-500"}`}>
          GST {u.gstNumber ? "✓" : "—"}
        </span>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="Search seller, ID, company..."
        ...
        {(filters.search || filters.date) && (
          <button
            onClick={handleReset}
            className="px-3.5 py-2 text-[10px] font-bold text-slate-400 hover:text-white border border-[#1e293b] rounded-xl transition cursor-pointer"
          >
            Reset
          </button>
        )}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#1e293b]">
                {["Seller", "Documents", "Submitted", "Status", "Action"].map((h) => (
                ...
                <tr>
                  <td colSpan={5} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs font-bold text-slate-400">No pending submissions found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                users.map((u) => {
                  const submittedDate = new Date(u.createdAt).toLocaleDateString("en-IN", {
                    day: "2-digit", month: "short", year: "numeric"
                  });
                  const submittedTime = new Date(u.createdAt).toLocaleTimeString("en-IN", {
                    hour: "2-digit", minute: "2-digit", hour12: true
                  });
                  return (
                    <tr key={u.id} className="border-b border-[#1e293b]/60 hover:bg-white/[0.01] transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-[11px] font-bold text-white">{u.firstName} {u.lastName}</p>
                            <p className="text-[9px] font-mono text-slate-500">{u.id?.slice(0, 8)?.toUpperCase()}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <p className="text-[11px] font-semibold text-slate-200">{submittedDate}</p>
                        <p className="text-[9px] text-slate-500 mt-0.5">{submittedTime}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[9px] font-bold uppercase tracking-wide">
                          Pending
                        </span>
                      </td>
                      <td className="px-4 py-3">
