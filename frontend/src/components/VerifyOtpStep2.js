...
export default function VerifyOtpStep2({ 
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm w-full flex flex-col gap-6 relative">
      ...
      <button 
        onClick={onBack}
        className="absolute top-4 left-4 text-slate-400 hover:text-slate-700 transition text-xs font-semibold cursor-pointer flex items-center gap-1"
      >
        ← Back
      </button>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div>
          <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mb-1.5 text-center">6-Digit Verification Code</label>
          <input
            type="text"
            maxLength={6}
            value={code}
