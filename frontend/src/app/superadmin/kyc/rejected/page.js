...
export default function RejectedKYCPage() {
  ...
  const fetchRejectedList = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const q = new URLSearchParams({ page: String(page), limit: "100", status: "REJECTED" });
      ...
      console.error("Failed to load rejected KYC list:", err);
    } finally {
      setLoading(false);
    }
  }, [filters]);
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#1e293b]">
                {["Seller", "Company", "Rejection Reason", "Status", "Action"].map((h) => (
              ...
              {loading ? (
                ...
                users.map((u) => (
                  <tr key={u.id} className="border-b border-[#1e293b]/60 text-xs hover:bg-white/[0.01]">
                    ...
                    <td className="px-4 py-3 text-rose-400 font-medium max-w-xs truncate" title={u.kycRejectReason}>{u.kycRejectReason || "—"}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 text-[9px] font-bold uppercase tracking-wide">
                        Rejected
                      </span>
                    </td>
                    <td className="px-4 py-3">
