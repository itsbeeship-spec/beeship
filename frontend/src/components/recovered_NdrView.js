...
export default function NdrView() {
  ...
  const [expandedRows, setExpandedRows] = useState(["1"]); // By default expanded row 1 (Manish Garu)
  ...
  // Get status count for Action Required (badge style)
  ...
  return (
    <div className="w-full animate-fadeIn select-none">
      ...
      <div className="flex justify-between items-center w-full mb-6">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-2 px-4.5 py-2.5 bg-[#1b2438] hover:bg-slate-800 text-white hover:shadow-md rounded-xl text-xs font-semibold shadow-sm transition cursor-pointer"
          >
          ...
          <div className="grid grid-cols-12 gap-5 items-end">
            ...
            <div className="col-span-12 md:col-span-3 relative" ref={dateRef}>
              ...
              {showCustomPicker && (
                <div className="absolute left-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 z-45 animate-fadeIn min-w-[530px] font-sans">
                  ...
                  <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-4 text-[11px] font-bold text-slate-600">
                    <span>
            ...
            <div className="col-span-12 flex justify-end gap-3 mt-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4.5 py-2.5 bg-[#1b2438] hover:bg-slate-800 text-white hover:shadow-md rounded-xl text-xs font-semibold shadow-sm transition cursor-pointer"
              >
            ...
            >
              ...
              {tab.count > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                  isActive ? "bg-white/20 text-white" : "bg-[#1b2438]/10 text-[#1b2438]"
                }`}>
                  {tab.count}
                </span>
              )}
              ...
              <tr className="border-b border-slate-100 bg-slate-50/20 text-slate-400 font-bold uppercase tracking-wider text-[9px] select-none">
                ...
                <th className="py-3 pr-5 pl-4 text-right font-bold">
                  <div className="flex items-center gap-1.5 justify-end">
                    <svg className="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.54 0 2.9-.826 2.37-2.37M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    <span>Action</span>
                  </div>
                </th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700 text-xs font-semibold">
              {displayedReports.length > 0 ? (
                displayedReports.map((report) => {
                  ...
                  return (
                    <tr key={report.id} className="hover:bg-slate-50/20 transition-all flex flex-col md:table-row">
                      {/* Checkbox & Dropdown Arrow row cell */}
                      <td className="py-3 px-5 text-center w-10 flex items-center gap-2 md:table-cell">
                        <input
                          type="checkbox"
                          checked={isSelected}
                        ...
                        >
                          <svg className={`w-3 h-3 transform transition-transform duration-200 ${isExpanded ? "rotate-180" : "-rotate-90"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" />
                          </svg>
                        </button>
                      </td>
                      ...
                      <td className="py-3 px-4 md:table-cell">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {report.method === "COD" && (
                              <span className="text-[10px] text-emerald-600 font-bold">₹{report.collectable}</span>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      {/* Products info cell */}
                      <td className="py-3 px-4 md:table-cell truncate max-w-[130px]" title={report.products.map(p => p.name).join(", ")}>
                      ...
                      <td className="py-3 pr-5 pl-4 text-right md:table-cell relative">
                        <button
                          type="button"
                          onClick={() => setActionDropdownId(actionDropdownId === report.id ? null : report.id)}
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-[#1b2438] hover:bg-slate-800 text-white rounded-lg text-[10px] font-bold shadow-xs transition cursor-pointer"
                        >
                          <span>Actions</span>
      ...
      {displayedReports.length > 0 && displayedReports.map((report) => {
        ...
        return (
          <div key={`expanded-${report.id}`} className="bg-slate-50/20 border-x border-b border-slate-200/80 rounded-b-2xl p-6.5 -mt-px flex flex-wrap md:grid md:grid-cols-12 gap-6 transition-all animate-fadeIn">
            {/* SHIPPING INFO COLUMN */}
            <div className="col-span-12 md:col-span-3 flex flex-col gap-3 font-semibold text-xs text-slate-700">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 font-medium leading-relaxed max-w-[220px]">
                  <svg className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  </svg>
                  <span className="text-slate-500 text-[10.5px]">{report.address}</span>
                </div>
              </div>
            </div>

            {/* PRODUCTS COLUMN */}
            <div className="col-span-12 md:col-span-4 flex flex-col gap-3 font-semibold text-xs text-slate-700">
              <span className="text-slate-400 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5">
                <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                </svg>
              ...
              <div className="flex flex-col gap-3 max-h-36 overflow-y-auto pr-2">
                {report.products.map((p, idx) => (
                  <div key={idx} className="flex gap-2 items-start text-slate-800 leading-normal">
            ...
            <div className="col-span-12 md:col-span-2 flex flex-col gap-3 font-semibold text-xs text-slate-700 md:border-l border-slate-200/80 md:pl-5">
              <span className="text-slate-400 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5">
                <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
