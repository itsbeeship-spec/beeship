...
export default function DocNotSubmittedPage() {
  const [users, setUsers] = useState([]);
  const [meta, setMeta] = useState({ total: 0, page: 1, limit: 100, counts: { PENDING: 0, APPROVED: 0, REJECTED: 0, NOT_SUBMITTED: 0 } });
  const [loading, setLoading] = useState(true);
  const [filters, setFilters] = useState({ search: "" });

  const fetchNotSubmittedList = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const q = new URLSearchParams({ page: String(page), limit: "100", status: "NOT_SUBMITTED" });
      if (filters.search) q.append("search", filters.search);
      const res = await api.get(`/admin/kyc/pending?${q.toString()}`);
      if (res && res.success) {
        setUsers(res.data || []);
        setMeta(res.meta || { total: 0, page: 1, limit: 100, counts: { PENDING: 0, APPROVED: 0, REJECTED: 0, NOT_SUBMITTED: 0 } });
      }
    } catch (err) {
      console.error("Failed to load not submitted KYC list:", err);
    } finally {
      setLoading(false);
    }
  }, [filters]);
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-[#1e293b]">
                  {["Seller", "Company", "Email", "Phone", "Status"].map((h) => (
                  ...
                  users.map((u) => (
                    <tr key={u.id} className="border-b border-[#1e293b]/60 text-xs hover:bg-white/[0.01]">
                      ...
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700/50 text-[9px] font-bold uppercase tracking-wide">
                          Not Submitted
                        </span>
                      </td>
                    </tr>
